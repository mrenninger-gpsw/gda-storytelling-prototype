...
import flash.display.Shape;
	...
	public class Footer extends Sprite {
        ...
        private var _bumpersBtn:FooterBtn;
		private var _previewBtn:FooterBtn;
        private var _saveBtn:FooterBtn;
        private var _cancelBtn:FooterBtn;
		...
		private function _init():void {
            ...
            var __bmp:Bitmap;
            __bmp = Register.ASSETS.getBitmap('FooterBtn-MusicIcon');
            _changeToMusicBtn = new FooterBtn(__bmp.width, __bmp.height, __bmp, false);//new UITransitionBtn('footer_musicIcon');
            TweenMax.to(_changeToMusicBtn, 0, {autoAlpha:1, x:Register.APP.WIDTH/2 - __bmp.width - 18, y:(bgShape.height - _changeToMusicBtn.height) * 0.5});
            this.addChild(_changeToMusicBtn);
            _enableBtn(_changeToMusicBtn);

            __bmp = Register.ASSETS.getBitmap('FooterBtn-BumpersIcon');
            _bumpersBtn = new FooterBtn(__bmp.width, __bmp.height, __bmp, false);//new UITransitionBtn('footer_musicIcon');
            TweenMax.to(_bumpersBtn, 0, {autoAlpha:1, x:Register.APP.WIDTH/2 + 18, y:(bgShape.height - _bumpersBtn.height) * 0.5});
            this.addChild(_bumpersBtn);
            _enableBtn(_bumpersBtn);
            ...
            _cancelBtn = new FooterBtn(115, 44, Register.ASSETS.getBitmap('AdvEditor_Text-Cancel'), false);
            TweenMax.to(_cancelBtn, 0, {x:_saveBtn.x - _saveBtn.width - 21, y:(bgShape.height - _cancelBtn.height) * 0.5});
		...
		private function _switchStates($id:String):void {
			...
			switch ($id) {
				case 'music':
                    ...
                    TweenMax.to(_cancelBtn, 0.3, {autoAlpha:0, delay:0.15});
                    TweenMax.to(_saveBtn, 0.3, {autoAlpha:0, delay:0.2});
					break;
				...
				case 'video':
                    ...
                    TweenMax.to(_cancelBtn, 0.3, {autoAlpha:1, delay:0.15});
                    TweenMax.to(_saveBtn, 0.3, {autoAlpha:1, delay:0.2});
					break;
			}
